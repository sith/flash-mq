...
handle_cast({listener,Listener},#queue{name = Name,listeners=[]} = Queue) ->
    {noreply, Queue#queue{listeners = [Listener]}};

handle_cast({listener,Listener},#queue{name = Name,listeners=Listeners} = Queue) ->
    {noreply, Queue#queue{listeners = Listeners ++ Listener}};


handle_cast({send,Message},#queue{listeners=[]} = Queue) -> {noreply, Queue};
handle_cast({send,Message},#queue{listeners=[H|_]} = Queue) ->
    H ! Message,
    {noreply, Queue}.
